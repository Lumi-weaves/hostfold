...
def validate_rendered_ssh(model: Model, view_name: str, config_path: Path) -> None:
    ...
    for destination in sorted(view.routes):
...
def _render_known_hosts(model: Model, view_name: str) -> str:
    lines = ["# Generated by Hostfold. Keys are pinned to canonical aliases."]
    for node_name in sorted(model.views[view_name].routes):
...
def _build_receipt(model: Model, view_name: str, staging: Path) -> dict[str, Any]:
    ...
    core: dict[str, Any] = {
        ...
        "ssh_hosts": {
            ...
            for destination in sorted(model.views[view_name].routes)
        },
        "files": files,
    }
